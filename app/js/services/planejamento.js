...
async function planejamentoSyncCache(){
  ...
  try{
    ...
    const {data,error}=await window.coiDB.from('planejamento_lancamentos').select('*').order('criado_em',{ascending:true});
...
async function planejamentoCriar(dados){
  ...
  const {data:inserted,error}=await window.coiDB.from('planejamento_lancamentos').insert(row).select('*').single();
...
async function planejamentoAtualizar(grupoId,dados){
  ...
  const {error:updErr}=await window.coiDB.from('planejamento_lancamentos').update({atual:false}).eq('id',atualLocal.id);
  ...
  const {data:inserted,error}=await window.coiDB.from('planejamento_lancamentos').insert(row).select('*').single();
  if(error){
    await window.coiDB.from('planejamento_lancamentos').update({atual:true}).eq('id',atualLocal.id);
...
async function planejamentoExcluir(grupoId){
  ...
  const {error}=await window.coiDB.from('planejamento_lancamentos').update({status:'excluido'}).eq('id',atualLocal.id);
